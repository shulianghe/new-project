...
#define NET_ERR_GETTING_ZOOMS						1203 // 正在获取防区参数
...
#define	NET_ERR_NOT_SUPPORT_OPERATE_ZOOM			1210 // 防区不支持该操作
...
#define	NET_ERR_ZOOM_FAULT_STATUS					1216 // 防区处于故障状态
...
#define		NET_DVR_GET_DISP_SCREEN_CFG			1625	//获取显示屏配置
#define		NET_DVR_SET_DISP_SCREEN_CFG			1626	//设置显示屏配置
...
#define	 NET_DVR_GET_ALARMHOST_ZOOM_LIST_IN_SUBSYSTEM		2034			//获取指定子系统内的所有防区
...
#define NET_DVR_CONTROL_RESTORE_SUPPORT  3311 // 	快球机芯恢复默认配置
...
#define MINOR_RUN_STATUS_INFO           0x87 /*系统运行状态信息*/
...
{
    DWORD   dwSize;						//结构长度
    BYTE	byResultType;			//0-视频识别结果，1-图像识别结果
...
typedef struct tagNET_DVR_SUBSYSTEM_ABILITY
{
	...
	union
	{
		...
		struct
		{
			...
			BYTE  bySupportThirdPartStream;	//是否支持第三方解码（万能解码）
            BYTE  byRes2[152];
...
typedef struct _NET_DVR_PUBLIC_SUB_SYSTEM
{
...
typedef struct _NET_DVR_NOAMAL_SUB_SYSTEM
{
...
typedef union _NET_DVR_JOINT_SUB_SYSTEM
{
...
typedef struct tagNET_DVR_ALARMSUBSYSTEMPARAM
{
	...
	BYTE		byKeyZoomArm;		//是否支持钥匙防区对子系统进行布防操作
	BYTE		byKeyZoomArmReport;	//是否支持钥匙防区对子系统布防时发送布防报告
	BYTE		byKeyZoomDisarm;	//是否支持钥匙防区对子系统进行撤防操作
	BYTE		byKeyZoomDisarmReport;	//是否支持钥匙防区对子系统进行撤防操作时发送撤防报告
...
typedef struct _tagNET_DVR_ALARMHOST_WIRELESS_NETWORK_CFG
{
	DWORD	dwSize;
...
#define LONG_CFG_TYPE_FIND_NAS_DIRECTORY          6161        //查找NAS目录
...
//NET_DVR_Set/GetDVRConfig 通道参数 0表示对设备进行配置, 1以上表示对应用的通道,目前1以上无效.
...
typedef struct _tagNET_DVR_FTP_TYPE
{
	BYTE	byType;  // 0-主服务器，1-备服务器
	BYTE	byRes[3];
...
typedef struct tagNET_DVR_FOCUSMODE_CFG
{
	...
	BYTE  byAutoFocusMode; /* 自动聚焦模式，0-关，1-模式A，2-模式B，3-模式AB，4-模式C */
...
//获取干线状态
typedef struct _tagNET_DVR_TRUNK_USE_STATE
{
	DWORD		dwSize;
